...
for symbol in symbols:
    ...
    # Create a data feed
    dataframe = btest.feeds.pull("OPTIONS",symbol, timeframe, fromDate=from_date, toDate=to_date)
    ...
    if strategyName == 'KingKeltner':
        cerebro.addstrategy(strategy.KingKeltner) 
    elif strategyName == 'BollingerBandit':
        cerebro.addstrategy(strategy.BollingerBandit) 
    elif strategyName == 'VWAPCrossover':
        #cerebro.addstrategy(strategy.VWAPCrossover) 
        cerebro.addstrategy(strategy.SMACROSSOVER)
    ...
    strategies = cerebro.run(tradehistory=True, runonce=False)
